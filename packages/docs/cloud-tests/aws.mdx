...
description: "AWS Cloud Tests allows you to connect your AWS infrastructure to Compiel for automated security testing using Security Hub, compliance monitoring, and risk assessment."
---
...
3. **Choose a Region Code**
   - Decide which AWS region you want Compiel to use (e.g., `us-east-1`, `us-west-2`)
   - Copy this region code — you’ll need it in the Compiel UI
4. **Create an IAM User for Compiel**
   - Go to **IAM \> Users**, then click **Add user**
   - Enter a name like `CompielIntegrationUser`
...
7. **Connect AWS to Compiel**
   - Go to **Settings \> Integrations** in your Compiel dashboard
...
Compiel uses a cross-account IAM role with read-only permissions and a required external ID to ensure secure, scoped access. This approach follows AWS best practices for secure third-party integrations.
...
1. Check our [Knowledge Base](https://help.trycompiel.com/azure-integration)
